...
int main(int argc, char *argv[])
{
    ...
    char* message = "the rain is in spain";

   size_t len = strlen(message) + 1; /* +1 to include null char */
	size_t ctLen = ske_getOutputLen(len);
	char* emessage = malloc(ctLen);
    unsigned char errMask;
	size_t s = ske_encrypt(emessage,message,len,&K,IV);
    #if VDEBUG
	size_t i;
	for (i = 0; i < ctLen; i++) {
		fprintf(stderr, "%02x",emessage[i]);
	}
	fprintf(stderr, "\n");
	#endif
	/* now try to decrypt it. */
	char* dmessage = malloc(len);
	/* tweak ct and see what happens: */
	emessage[rand() % ctLen] ^= errMask;
	size_t r = ske_decrypt(dmessage,emessage,ctLen,&K);
    BIO_dump_fp (stdout, (const char *)emessage, s);
    fprintf( stderr, "%s", dmessage );
	return 0;
}  
